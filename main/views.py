    ...
    if request.method == 'POST':
        try:
            ...
            if latitud and longitud:
                # Procesar o almacenar la ubicación
                print(f"Ubicación recibida: Latitud {latitud}, Longitud {longitud}")
                return JsonResponse({'mensaje': 'Ubicación recibida correctamente.'})
            else:
                return JsonResponse({'error': 'Faltan datos de ubicación.'}, status=400)

        except Exception as e:
            print(f"Error al procesar la ubicación: {e}")
            return JsonResponse({'error': 'Error al procesar la ubicación.'}, status=500)
